...
export const useRawKiesMbo = async (opt?: Options) => {
    const { filterByCreboCodes = false } = opt || {};
    ...
    const { creboCodes } = useConfig().kiesmbo;
    if (filterByCreboCodes) {
...
export const useKiesMbo = async (opt?: Options) => {
    const { filterByCreboCodes = false } = opt || {};
    const { _organizations, _products, _locations } = await useRawKiesMbo({filterByCreboCodes});
